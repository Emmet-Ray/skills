...
class CodexOAuthImageProvider(ImageProvider):
    ...
    def _post_json(self, path: str, body: Dict[str, Any]) -> Dict[str, Any]:
        if self._urlopen is default_urlopen:
            return self._post_json_httpx(path, body)
    ...
    def _post_json_httpx(self, path: str, body: Dict[str, Any]) -> Dict[str, Any]:
        url = f"{self.codex_base_url}/{path}"
        try:
            ...
            raise RuntimeError(f"Codex Images request failed: {exc}") from exc

        if resp.status_code >= 400:
            detail = resp.text[:4096]
            raise CodexOAuthHTTPError("Codex Images request", resp.status_code, detail)
        if len(resp.content) > MAX_CODEX_RESPONSE_BYTES:
            raise RuntimeError("Codex image response exceeded size limit.")
