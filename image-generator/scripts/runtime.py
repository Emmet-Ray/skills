#!/usr/bin/env python3
"""Manage the shared runtime for the image-generator skill.

The runtime lives outside the repository so multiple local checkouts can share
one virtual environment and one API configuration.
"""
...
def _load_codex_access_token(path: Path) -> Optional[str]:
    if not path.exists():
        return None
    try:
...
def _write_env_file(path: Path, values: Dict[str, str]) -> None:
    ...
    lines = [
        "# image-generator shared runtime configuration",
        "# Used by image_generator/scripts/image_gen.py and runtime.py.",
    ]
...
def _load_env_values(home: Path) -> Dict[str, str]:
    ...
    for key in (*ENV_FIELDS, *LEGACY_ENV_FIELDS):
        if os.getenv(key):
...
def _config(args: argparse.Namespace) -> int:
    ...
    if args.model is not None:
        values["IMAGE_GENERATOR_IMAGE_MODEL"] = args.model.strip()
    if args.backend is not None:
        values["IMAGE_GENERATOR_IMAGE_BACKEND"] = args.backend.strip()
...
def _check_python_imports(python: Path) -> bool:
    if not python.exists():
        ...
        return False
    code = "import openai, httpx; print('imports ok')"
    proc = subprocess.run(
        [str(python), "-c", code],
...
def _doctor(args: argparse.Namespace) -> int:
    ...
    model = (
        values.get("IMAGE_GENERATOR_IMAGE_MODEL")
        or values.get("IMAGE2_IMAGE_MODEL")
        or values.get("CODEX_PPT_IMAGE_MODEL", DEFAULT_MODEL)
    )
    backend = values.get("IMAGE_GENERATOR_IMAGE_BACKEND") or values.get(
        "IMAGE2_IMAGE_BACKEND"
    ) or values.get(
        "CODEX_PPT_IMAGE_BACKEND",
    ...
    codex_ready = _load_codex_access_token(auth_file) is not None

    print(f"OPENAI_API_KEY={'set (' + _mask_secret(api_key) + ')' if api_key else '<unset>'}")
    print(f"OPENAI_BASE_URL={base_url}")
    print(f"IMAGE_GENERATOR_IMAGE_MODEL={model}")
    print(f"IMAGE_GENERATOR_IMAGE_BACKEND={backend}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage the image-generator shared runtime")
