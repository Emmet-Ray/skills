#!/usr/bin/env python3
"""Standalone CLI for image generation or editing with GPT Image models.
...
Reads OPENAI_API_KEY, and optionally OPENAI_BASE_URL for OpenAI SDK routing.
"""
...
MAX_BATCH_JOBS = 500
DEFAULT_RUNTIME_HOME = "~/.image-generator"
ENV_FIELDS = (
    "OPENAI_API_KEY",
    "OPENAI_BASE_URL",
    "IMAGE_GENERATOR_IMAGE_MODEL",
    "IMAGE_GENERATOR_IMAGE_BACKEND",
    "IMAGE2_IMAGE_MODEL",
    "IMAGE2_IMAGE_BACKEND",
    "CODEX_PPT_IMAGE_MODEL",
    "CODEX_PPT_IMAGE_BACKEND",
    "CODEX_IMAGES_BASE_URL",
)
VALID_IMAGE_BACKENDS = ("auto", "codex-oauth", "openai")
...
def _runtime_home() -> Path:
    return Path(
        os.getenv("IMAGE_GENERATOR_HOME")
        or os.getenv("IMAGE2_HOME")
        or os.getenv("CODEX_PPT_HOME", DEFAULT_RUNTIME_HOME)
    ).expanduser()
...
def _default_model() -> str:
    return (
        os.getenv("IMAGE_GENERATOR_IMAGE_MODEL")
        or os.getenv("IMAGE2_IMAGE_MODEL")
        or os.getenv("CODEX_PPT_IMAGE_MODEL", DEFAULT_MODEL)
    )


def _default_backend() -> str:
    return (
        os.getenv("IMAGE_GENERATOR_IMAGE_BACKEND")
        or os.getenv("IMAGE2_IMAGE_BACKEND")
        or os.getenv("CODEX_PPT_IMAGE_BACKEND", "auto")
    )
...
def _api_target_label(backend: Optional[str] = None) -> str:
    ...
    if base_url:
        return f"OpenAI SDK image API (OPENAI_BASE_URL={base_url})"
...
def _ensure_provider_config(provider: ImageProvider, dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        return
    runtime_script = _skill_root() / "runtime.py"
    skill_doc = _skill_root().parent / "SKILL.md"
    base_url = _api_base_url()
    model = _default_model()
    if base_url:
        command = (
            f'python {runtime_script} config --api-key "your-api-key" '
            f'--base-url "{base_url}" --model {model}'
        )
        target_hint = f"Detected OpenAI SDK API target via OPENAI_BASE_URL={base_url}."
    else:
        command = f'python {runtime_script} config --api-key "your-api-key" --model {model}'
        target_hint = "Detected official OpenAI API mode because OPENAI_BASE_URL is not set."
    _die(
        "OPENAI_API_KEY is not set for the selected image provider.\n"
        f"{target_hint}\n"
        "Configure the shared runtime once, or use --backend auto with Codex OAuth auth available:\n"
        f"  {command}\n"
        "To use a custom OpenAI SDK base URL, set OPENAI_BASE_URL and the provider's model name.\n"
        f"Details: {skill_doc}"
    )
        ...
        _print_request(
            {
                ...
                **{
                    **payload_preview,
                    "model": str(payload_preview["model"]),
                },
            }
        )
        return
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Standalone CLI for explicit image generation or editing via GPT Image models"
    )
